(ns metav.release-test
  (:require [testit.core :refer :all]
            ...
            [metav.display :refer [version module-name artefact-name *separator*]]
            [metav.git :as git]
...
(deftest release-repo
  (testing "bump from a clean tagged repo"
    (let [[monorepo remote moduleA1 moduleA2 moduleB1 moduleB2] (monorepo);module A1 latest tag is 1.3.4
          ...
          latest-tag (git/latest-tag monorepo)
          [scm-base distance sha dirty?] (git/working-copy-description moduleA1 :prefix (str module-name *separator* ))]
      (facts
         (str bumped-version) => "1.3.5"
         scm-base => "1.3.5"
         latest-tag =>  "sysA-container1-1.3.5")
      (fs/delete monorepo)
      )))
